...
export default {
  ...
  async function bulkCreateAds(request, env) {
    try {
      ...
      const cleanSparkCodes = (sparkCodes || []).map(code => (code || '').trim()).filter(Boolean);
      const cleanTiktokLinks = (tiktokLinks || []).map(link => (link || '').trim()).filter(Boolean);
      
      // Determine the count - at least one item should exist
      const count = Math.max(cleanSparkCodes.length, cleanTiktokLinks.length, 1);
      
      if (count === 0) {
        return new Response(JSON.stringify({ error: 'At least one spark code or TikTok link is required' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' }
        });
      }
      ...
      // Generate spark codes if missing
      const generateSparkCode = (index) => {
        const timestamp = Date.now();
        const random = Math.random().toString(36).substring(2, 6).toUpperCase();
        return `AUTO-${baseName.split('-')[0].toUpperCase()}-${String(index + 1).padStart(2, '0')}-${random}`;
      };
      
      for (let i = 0; i < count; i++) {
        try {
          // Auto-generate spark code if missing
          let sparkCode = cleanSparkCodes[i] || '';
          if (!sparkCode && cleanTiktokLinks[i]) {
            // Only auto-generate if there's a TikTok link
            sparkCode = generateSparkCode(i);
          }
          ...
          const ad = {
            ...
            sparkCode: sparkCode,
            tiktokLink: cleanTiktokLinks[i] || '',
            postId: extractPostId(cleanTiktokLinks[i] || ''),
            type: type,
            status: 'New',
            winner: 'Unknown',
            notes: sparkCode.startsWith('AUTO-') ? 'Auto-generated spark code' : '',
          ...
          errors.push(`Failed to create ad ${i + 1}: ${err.message}`);
        }
      }
      ...
      if (cleanSparkCodes.length !== cleanTiktokLinks.length) {
        response.warnings.push(`Mismatched counts: ${cleanSparkCodes.length} spark codes and ${cleanTiktokLinks.length} TikTok links`);
      }
      
      const autoGeneratedCount = created.filter(ad => ad.sparkCode.startsWith('AUTO-')).length;
      if (autoGeneratedCount > 0) {
        response.warnings.push(`Auto-generated ${autoGeneratedCount} spark codes`);
      }
  ...
  <head>
      ...
      <style>
          * {
          ...
          /* Tooltip styles for spark codes */
          [title] {
              position: relative;
          }
          ...
          .spark-code-cell:hover .spark-code-full {
              display: block;
          }
  ...
  <body>
          ...
          <div id="payments-section" class="payment-section">
              ...
              <div class="payment-settings">
                  ...
                  <div class="rate-input">
                      ...
                      <input type="number" id="default-rate" value="1" step="0.01">
                      <button id="save-default-rate" onclick="saveDefaultRate()">Save Rate</button>
                  </div>
      ...
      <script>
          ...
          // Truncate long spark codes for display
          function truncateSparkCode(code, maxLength = 20) {
              if (!code) return '-';
              if (code.length <= maxLength) return code;
              return code.substring(0, maxLength) + '...';
          }
          
          // Create spark code display with tooltip for long codes
          function getSparkCodeDisplay(sparkCode) {
              if (!sparkCode) return '-';
              const truncated = truncateSparkCode(sparkCode);
              if (sparkCode.length > 20) {
                  // Escape quotes in the spark code for the title attribute
                  const escapedCode = sparkCode.replace(/"/g, '&quot;');
                  return \`<span title="\${escapedCode}" style="cursor: help; border-bottom: 1px dotted #00f2ea;" onclick="copyToClipboard('\${escapedCode.replace(/'/g, '\\\\\'')}')">\${truncated}</span>\`;
              }
              return sparkCode;
          }
          ...
          function copyToClipboard(text) {
              ...
              // Prevent cell edit when copying
          ...
          function renderAdsTable() {
              ...
              filteredAds.forEach(ad => {
                  ...
                  row.innerHTML = \`
                      ...
                      <td onclick="editCell(this, '\${ad.id}', 'sparkCode')" data-full-value="\${ad.sparkCode || ''}">\${getSparkCodeDisplay(ad.sparkCode)}</td>
          ...
          function editCell(cell, adId, field) {
              ...
              // For spark codes, get the full value from data attribute or the ad object
              const currentValue = field === 'sparkCode' && cell.dataset.fullValue ? 
                                   cell.dataset.fullValue : (ad[field] || '');
          ...
          function previewBulkAds() {
              ...
              const count = Math.max(sparkCodes.length, tiktokLinks.length, 1);
              
              // Add summary
              const summaryDiv = document.createElement('div');
              summaryDiv.style.marginBottom = '10px';
              summaryDiv.style.color = '#00f2ea';
              summaryDiv.innerHTML = \`<strong>Creating \${count} ads</strong> - \${sparkCodes.length} spark codes, \${tiktokLinks.length} TikTok links\`;
              previewContent.appendChild(summaryDiv);
              
              for (let i = 0; i < count; i++) {
                  const name = \`\${baseName}-\${String(i + 1).padStart(2, '0')}\`;
                  const sparkCode = sparkCodes[i];
                  const tiktokLink = tiktokLinks[i];
                  ...
                  let sparkDisplay;
                  if (sparkCode) {
                      // Truncate long spark codes in preview
                      sparkDisplay = sparkCode.length > 30 ? 
                                    \`<span title="\${sparkCode}">\${sparkCode.substring(0, 30)}...</span>\` : 
                                    sparkCode;
                  } else {
                      sparkDisplay = tiktokLink ? '🔄 Auto-generate' : '❌ Missing';
                  }
                  let linkDisplay = tiktokLink ? '✓ Provided' : '❌ Missing';
                  
                  div.innerHTML = \`<strong>\${name}</strong> - Spark: \${sparkDisplay} - TikTok: \${linkDisplay}\`;
                  
                  // Highlight issues
                  if (!sparkCode && !tiktokLink) {
                      div.style.backgroundColor = '#dc26261a';
                      div.style.border = '1px solid #dc2626';
                  } else if (!sparkCode && tiktokLink) {
                      div.style.backgroundColor = '#f59e0b1a';
              ...
              if (sparkCodes.length !== tiktokLinks.length) {
                  ...
                  warningDiv.style.backgroundColor = '#f59e0b1a';
                  warningDiv.style.border = '1px solid #f59e0b';
                  warningDiv.style.borderRadius = '4px';
                  warningDiv.innerHTML = \`⚠️ Mismatched counts: Spark codes will be auto-generated for TikTok links without codes\`;
          ...
          async function saveBulkAds() {
              ...
              const hasSparkCodes = sparkCodes.some(code => code.length > 0);
              const hasTiktokLinks = tiktokLinks.some(link => link.length > 0);
              
              if (!hasSparkCodes && !hasTiktokLinks) {
                  showError('Please provide at least one spark code or TikTok link');
                  return;
              }
              ...
              try {
                  ...
                  if (!response.ok) {
                      ...
                      throw new Error(error.error || 'Failed to create ads');
                  }
                  
                  const result = await response.json();
                  const created = result.created || result; // Handle both old and new response formats
                  ...
                  if (Array.isArray(created)) {
                      ...
                      let successMsg = \`Created \${created.length} ads successfully\`;
                      ...
                      if (result.errors && result.errors.length > 0) {
                          setTimeout(() => {
                              showError(\`Some issues occurred: \${result.errors.join(', ')}\`);
                          }, 500);
                      }
                  } else {
                  ...
                  showError(error.message || 'Failed to create ads. Please try again.');
              } finally {
          ...
          function renderCreatorRates() {
              ...
              creators.forEach(creator => {
                  ...
                  div.innerHTML = \`
                      <label>\${creator}: $</label>
                      <input type="number" id="rate-\${creator}" value="\${settings.creatorRates[creator] || settings.defaultRate}" 
                             step="0.01">
                      <button onclick="saveCreatorRate('\${creator}')">Save</button>
                  \`;
          ...
          async function saveDefaultRate() {
              ...
              const newRate = parseFloat(document.getElementById('default-rate').value);
              ...
              // Show saving state
              const button = document.getElementById('save-default-rate');
              ...
              try {
                  ...
                  messageDiv.textContent = 'Default rate saved successfully!';
                  setTimeout(() => {
                      messageDiv.style.display = 'none';
                  }, 3000);
              } catch (error) {
                  showError('Failed to save rate. Please try again.');
              } finally {
          ...
          // Save creator rate
          async function saveCreatorRate(creator) {
              if (!isAdmin) {
                  showError('Only administrators can update payment rates.');
                  return;
              }
              
              const input = document.getElementById(\`rate-\${creator}\`);
              ...
              settings.creatorRates[creator] = newRate;
