...
import {
  ...
  JOB_STATES
} from './comment-bot-queue.service.js';
...
async function saveOrderToDatabase(env, job, orderResponse) {
  try {
    ...
    const result = await env.COMMENT_BOT_DB.prepare(insertQuery)
      .bind(
        job.user_id,
        job.team_id,
    ...
    await addJobLog(env, job.job_id, 'error', 'Failed to save order to database', {
      error: error.message
    });
  }
}
 ...
 */
export async function processCronJobs(env, maxJobs = 1, ctx = null) {
  try {
    // Get next job from queue
    const job = await getNextJob(env);
    ...
    // Use ctx.waitUntil to keep the async job alive even after response is sent
    const jobPromise = processJobAsync(env, job).catch(error => {});

    if (ctx && ctx.waitUntil) {
      ctx.waitUntil(jobPromise);
    }
